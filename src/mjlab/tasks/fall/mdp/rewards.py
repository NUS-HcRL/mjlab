...
class ThresholdContactForcePenalty:
  """Continuous penalty as tracked body forces approach termination thresholds."""
  ...
  def _maybe_initialize(self, env: ManagerBasedRlEnv) -> bool:
    sensor: ContactSensor = env.scene[self.sensor_name]
    assert sensor.data.force is not None
    all_body_names = _get_sensor_body_names(sensor)
    if self._body_names is None:
      all_body_name_set = set(all_body_names)
      body_names = [
        name for name in self.body_force_thresholds if name in all_body_name_set
      ]
      self._body_names = body_names
      self._body_indexes = [all_body_names.index(name) for name in body_names]
  ...
  def __call__(self, env: ManagerBasedRlEnv) -> torch.Tensor:
    ...
    penalty = excess.square().sum(dim=-1)

    self._metric_steps += 1.0
    self._accumulate("threshold_penalty", penalty)
    self._accumulate("threshold_ratio_max", force_ratio.max(dim=-1).values)
    for idx, body_name in enumerate(self._body_names or ()):
      self._accumulate(
